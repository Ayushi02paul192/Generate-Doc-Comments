...
def process_file(file_path, llm_wrapper, args):
    if not os.path.isfile(file_path):
        print(f"File {utils.get_bold_text(file_path)} does not exist")
        return

    if utils.has_unstaged_changes(file_path):
        print(f"File {utils.get_bold_text(file_path)} has unstaged changes")
        return
    ...
    with open(file_path, "r") as file:
        ...
        treesitter_nodes = treesitter_parser.parse(file_bytes)

        for node in treesitter_nodes:
            method_name = utils.get_bold_text(node.name)

            if node.doc_comment:
                print(f"⚠️  Method {method_name} already has a doc comment. Skipping...")
                continue
    ...
    for original_code, generated_doc_comment in generated_doc_comments.items():
        utils.write_code_snippet_to_file(
            file_path, original_code, generated_doc_comment
        )

def process_directory(directory_path, llm_wrapper, args):
    ...
    for file_name in files:
        ...
        process_file(file_path, llm_wrapper, args)
...
def run():
    ...
    if args.gpt4:
        ...
        directory_or_file_path = args.dir

    if os.path.isfile(directory_or_file_path):
        process_file(directory_or_file_path, llm_wrapper, args)
    elif os.path.isdir(directory_or_file_path):
        process_directory(directory_or_file_path, llm_wrapper, args)
    else:
        sys.exit(f"{directory_or_file_path} is not a valid file or directory path.")
